  ...
  options.gui.rofi.todofi = with lib; {
    ...
    colors = {
      colorTitle = mkOption {
        type = types.str;
        default = "#A9B665";
        description = "A hex color for the title.";
      };
      colorShortcut = mkOption {
        type = types.str;
        default = "#7DAEA3";
        description = "A hex color for the shortcut.";
      };
      colorInfo = mkOption {
        type = types.str;
        default = "#EA6962";
        description = "A hex color for info.";
      };
      colorExample = mkOption {
        type = types.str;
        default = "#7DAEA3";
        description = "A hex color for info.";
      };
      ...
      markupProject = mkOption {
        type = types.str;
        default = "#7DAEA3";
        description = "A hex color for project highlighting.";
      };
      markupContext = mkOption {
        type = types.str;
        default = "#A9B665";
        description = "A hex color for context highlighting.";
      };
      markupTag = mkOption {
        type = types.str;
        default = "#7C6F64";
        description = "A hex color for tag highlighting.";
      };
      markupDue = mkOption {
        type = types.str;
        default = "#EA6962";
        description = "A hex color for due date highlighting.";
      };
    };
  };
...
ROFI_BIN="$(command -v ${config.programs.rofi.package}/bin/rofi)"
TODO_BIN=$(command -v ${pkgs.todo}/bin/todo.sh)
...
confirm() {
    ...
    if [[ $context ]]; then
        ...
        response=$(echo -e "Yes\nNo" | $ROFI_BIN -theme-str listview{lines:2;} -u 0 -a 1 -dmenu -mesg "$mesg" -i -p "$message")
    else
        response=$(echo -e "Yes\nNo" | $ROFI_BIN -theme-str listview{lines:2;} -u 0 -a 1 -dmenu -i -p "$message")
    fi
...
add() {
    ...
    new_todo=$(echo -e "< Cancel" | runrofi -theme-str listview{lines:1;} -dmenu -mesg "New todo
''${projcon}" -p "> ")
...
edit() {
    ...
    projcon=`getprojconheader`
    todo=$(runrofi -theme-str listview{lines:0;} -dmenu -mesg "Edit todo
...
option() {
    ...
    do
        if [[ ''${current_line:0:1} == 'x' ]]; then
            ...
            selection=$(echo -e "1. Mark Done|2. Edit|3. Edit priority|4. Remove priority|5. Delete" | runrofi -theme-str listview{lines:5;} -sep "|" -u 4 -a 0 -kb-accept-entry "Return" -mesg "Item: ''${current_line}" -dmenu -p "Action")
...
Ex:
  ...
  * Hide tasks that do not contains FOO: <span color='''''${COLOR_EXAMPLE}'>-FOO</span>"
    FILTER=$(runrofi -format f -theme-str listview{lines:0;} -kb-accept-entry "Return" -mesg "''${HEADER}" -dmenu -p "Term" -filter "$FILTER")
...
linescount() {
    ...
    if [[ $LINES_MODE == 'auto' ]]; then
        ...
        echo "-theme-str listview{lines:$count;}"
    fi
}
...
config() {
    ...
    selection=$(
        ...
        runrofi -sep "|" -theme-str listview{lines:3;} -u 0 -a 1 -kb-accept-entry "Return" -mesg "''${HELP}" -dmenu -p "Action"
    )
    val=$?
...
Todo.txt format: https://github.com/todotxt/todo.txt"
    ...
    selection=$(
        ...
        runrofi -sep "|" -theme-str listview{lines:6;} -u 0 -a 1 -kb-accept-entry "Return" -mesg "''${HELP}" -dmenu -p "Action"
    )
    val=$?
