...
let
  ...
  rnm = rofi.rofiNetworkManager;
in
{
  options.gui.rofi.rofiNetworkManager = with lib; {
    enable = mkEnableOption "Enable a plugin to manage network connections in Rofi.";

    colors = {
      enabled = mkOption {
        type = types.str;
        default = "#A3BE8C";
        description = "The color of networks and devices that are enabled.";
      };
      disabled = mkOption {
        type = types.str;
        default = "#D35F5E";
        description = "The color of networks and devices that are disabled.";
      };
    };
  };

  config = lib.mkIf (rofi.enable && rnm.enable) {
    home.file.".local/bin/rofiNetworkManager" = {
      enable = true;
      force = true;
      executable = true;
      text = /* bash */ ''
...
ENABLED_COLOR="${rnm.colors.enabled}"
DISABLED_COLOR="${rnm.colors.disabled}"
