...
{
  imports = [];
  options.mediatek = with lib; {
    enable = mkEnableOption "Enable drivers and software needed to read and write to devices using Mediatek SOCs.";
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    users.users.kent.extraGroups = [ "dialout" "plugdev" ];
  };
}
