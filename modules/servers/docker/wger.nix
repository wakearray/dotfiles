{ lib, config, domain, ... }:
{
  options.servers.docker.wger = {
    enable = lib.mkEnableOption "Enable wger";
    volumes = {
      static = lib.mkOption {
        type = lib.types.path;
  ...
  config =
    let
      ...
      postgres-data = "${cfg.postgres-data}";
      redis-data = "${cfg.redis-data}";
      celery-beat = "${cfg.celery-beat}";
    in
    lib.mkIf config.servers.docker.wger.enable {
